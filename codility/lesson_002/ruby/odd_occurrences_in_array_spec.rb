...
Results:
  ...
  - Correctness: 100%
  - Performance: 100%
  - Link: https://codility.com/demo/results/training39FXWV-YE9/
...
def solution(a)
  ...
  a.each { |i| list[i] += 1 }
  list.detect { |_, v| v.odd? }.first
end

RSpec.describe "OddOccurrencesInArray" do
  it { expect(solution([42])).to eql(42) }

  it { expect(solution([9, 3, 9])).to eql(3) }
  it { expect(solution([9, 3, 9, 3, 9, 7, 9])).to eql(7) }

  it { expect(solution(Array.new(999_998, 1) << 2)).to eql(2) }
end
